...
/// Module has some functions which provide some low level functions for Aurix microcontrollers. The functions
/// use inline assembly.
pub mod asm_calls;
...
extern "C" {
    /// Declaration of _exit function, which is implemented in inline assembly.
    ///
    /// _exit function turns off debugging mode and jumps into an infinite loop.
    pub fn _exit(status: u32) -> !;
}

/// Panic handler of embedded Rust project.
///
/// A default panic handler should be defined in embedded programs compiled without standard library (#![no_std] feature).
/// This panic handler calls [`_exit`] function.
#[panic_handler]
...
/// Macro defines a main entry point for embedded project.
///
/// The init routines are linked together with a main function. This macro creates an unmangled `main` function and calls the other function
...
/// Macro defines a pre_init entry point for embedded project.
///
/// The init routines are linked together with a pre_init function. This macro creates an unmangled `pre_init` function and calls the other function
/// given in path parameter. Pre_init function is called before data initialization.
///
/// ### Example
/// ``` rust
/// pre_init!(post_init_fn);
/// fn post_init_fn() -> ! {
///     loop {}
/// }
/// ```
...
/// Macro defines a post_init entry point for embedded project.
///
/// The init routines are linked together with a post_init function. This macro creates an unmangled `post_init` function and calls the other function
/// given in path parameter. Post_init function is called at the end of init section.
///
/// ### Example
/// ``` rust
...
/// This function should be called on error.
///
/// # Safety
///
/// This function calls `exit(0xff)` and exit function is an extern "C" function.
#[no_mangle]
pub unsafe fn abort() -> ! {
    _exit(0xff);
}
